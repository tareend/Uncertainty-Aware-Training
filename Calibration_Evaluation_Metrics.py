...
root_save_dir = './Calibration_Metrics' \
                '/{0}_Test_{1}_{2}'.format(model_name, Test_Number, Date)
########################################################################################################################################################################################################################################################################################

if not os.path.exists(root_save_dir):
    os.system('mkdir -p {0}'.format(root_save_dir))
save_dir = '{0}/Test_Number_{1}_Epochs_{2}_lr_{3}_nbF_{4}_codeDim_{5}_classif_weight_{6}_Testing_Nested/'.format(
...
for kfold in range(folds):
    ...
    with torch.no_grad():
        ...
        pred_labels_CRT = np.argmax(probs.cpu().data.squeeze().numpy(),
                                            axis=1)
    ...
    pred_labels_CRT_all.append(pred_labels_CRT)

    fpr, tpr, thresholds = roc_curve(Y_CRT[test_index], pred_labels_CRT_outer)
    idx = np.argmax(tpr - fpr)
    optimal_th = thresholds[idx]
    y_CRT_pred_zo_orig = np.zeros_like(pred_labels_CRT_outer)
    y_CRT_pred_zo_orig[pred_labels_CRT_outer > optimal_th] = 1
