...
for iteration in range(Iteration_Runs):
    ...
    save_dir = os.path.join(root_save_dir_iter,
                            'saved_weights_and_data_lr_{0}_nbF_{1}_codeDim_{2}_z_concat_dim_{3}_kld_weight{4}_classif_weight_{5}_m_{6}_alpha_{7}'.format(
                                lr,
                                num_filters,
                                code_dim,
                                z_concat_dim,
                                kld_weight,
                                classifier_weight, m_hyper, alpha))

    image_dir = os.path.join(root_save_dir_iter,
                             'images_lr_{0}_nbF_{1}_codeDim_{2}_z_concat_dim_{3}_kld_weight_{4}_classif_weight_{5}_m_{6}_alpha_{7}'.format(
                                 lr, num_filters,
                                 code_dim, z_concat_dim, kld_weight, classifier_weight, m_hyper, alpha))
    ...
    for train_indices, test_indices in skf.split(indices, Y_CRT):
        ...
        model_path = os.path.join(save_dir, 'K_Fold_{0}_num_hidden_{1}_kld_weight_{2}'.format(kskf, z_concat_dim, kld_weight))
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx in range(num_batches):
                ...
                for i in range(len(GT_samples)):
                    if GT_samples[i] == 1 and pred_labels_CRT_arg[this_batch_indices][i] == 1:
                        sum_TP += 1
                        indices_TP.append(i)
                    if GT_samples[i] == 0 and pred_labels_CRT_arg[this_batch_indices][i] == 0:
                        sum_TN += 1
                        indices_TN.append(i)
                    if GT_samples[i] == 1 and pred_labels_CRT_arg[this_batch_indices][i] == 0:
                        sum_FN += 1
                        indices_FN.append(i)
                    if GT_samples[i] == 0 and pred_labels_CRT_arg[this_batch_indices][i] == 1:
                        sum_FP += 1
                        indices_FP.append(i)
                ...
                for i in range(sum_TP):
                    Probs_TP = confidences[this_batch_indices][indices_TP[i]] + Probs_TP
                for i in range(sum_FP):
                    Probs_FP = confidences[this_batch_indices][indices_FP[i]] + Probs_FP
                for i in range(sum_TN):
                    Probs_TN = confidences[this_batch_indices][indices_TN[i]] + Probs_TN
                for i in range(sum_FN):
                    Probs_FN = confidences[this_batch_indices][indices_FN[i]] + Probs_FN


                if sum_FP == 0 and sum_FN == 0:
                    confid_loss = torch.tensor(m_hyper * alpha)
                elif sum_FP == 0 and sum_TN == 0:
                    confid_loss = torch.tensor(m_hyper * alpha)
                elif sum_FP == 0:
                    confid_loss = (1 / sum_FN) * max((Probs_FN - Probs_TN + m_hyper), 0)
                    confid_loss = confid_loss * alpha
                elif sum_FN == 0:
                    confid_loss = (1 / sum_FP) * max((Probs_FP - Probs_TP + m_hyper), 0)
                    confid_loss = confid_loss * alpha
                else:
                    confid_loss = (1 / sum_FP) * max((Probs_FP - Probs_TP + m_hyper), 0) + (1 / sum_FN) * max(
                        (Probs_FN - Probs_TN + m_hyper), 0)
                    confid_loss = confid_loss * alpha

                paired_confid_loss = torch.from_numpy(np.array(confid_loss))
                out_confid_loss.append(confid_loss)
                ...
                total_loss = kld_loss + bce_loss + classifier_loss_crt + paired_confid_loss
                ...
                paired_confid_loss_epoch += paired_confid_loss.item()
            ...
            writer.add_scalar('Paired_confid_loss_epoch Loss/train', paired_confid_loss_epoch / num_batches, epoch)
            ######################################################################
