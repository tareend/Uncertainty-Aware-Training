...
kld_weight = 0.001
classifier_weight = 2
confid_weighting_scale = 3
sample = 20

# Best params from find optimal #
Test_number = 1
type = '_CW_Thresh_Test_Set'
Date = 'Date'
...
for iteration in range(Iteration_Runs):
    ...
    save_dir = os.path.join(root_save_dir_iter,
                            'saved_weights_and_data_lr_{0}_nbF_{1}_codeDim_{2}_z_concat_dim_{3}_kld_{4}_classif_weight_{5}_cw_{6}'.format(
                                lr,
                                num_filters,
                                code_dim,
                                z_concat_dim,
                                kld_weight,
                                classifier_weight, confid_weighting_scale))

    image_dir = os.path.join(root_save_dir_iter,
                             'images_lr_{0}_nbF_{1}_codeDim_{2}_z_concat_dim_{3}_kld_{4}_classif_weight_{5}_cw_{6}'.format(
                                 lr, num_filters,
                                 code_dim, z_concat_dim, kld_weight, classifier_weight, confid_weighting_scale))
