...
class Decoder2D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.resnet(x)
        # x = nn.Sigmoid()(self.conv_final(x))
        x = self.conv_final(x)  # don't need sigmoid if doing softmax in loss function
...
# DL hyper parameters
# **********************************************************************************************************************
num_epochs = 300 # change the number of epochs for the optimal model # 31 March work and updates
batch_size = 8
lr = 1e-4
num_filters = 32
code_dim = 128
...
Test_number = 0 ## Optimal Test from inner folds - depends on which outcome obtained
type = '_Base_Thresh_Test_Set'
Date = 'Date'
...
for iteration in range(Iteration_Runs):
    ...
    save_dir = os.path.join(root_save_dir_iter,
                            'saved_weights_and_data_lr_{0}_nbF_{1}_codeDim_{2}_z_concat_dim_{3}_kld_{4}_classif_weight_{5}'.format(
                                lr,
                                num_filters,
                                code_dim,
                                z_concat_dim,
    ...
    image_dir = os.path.join(root_save_dir_iter,
                             'images_lr_{0}_nbF_{1}_codeDim_{2}_z_concat_dim_{3}_kld_{4}_classif_weight_{5}'.format(
                                 lr, num_filters,
                                 code_dim, z_concat_dim, kld_weight, classifier_weight))
