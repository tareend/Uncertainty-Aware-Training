        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx in range(num_batches):
                ...
                for i in range(sum_pos):
                    Probs_pos_resp+=predicted_probs[indices_pos[i], 1].cpu().data.squeeze().numpy()
                for i in range(sum_neg):
                    Probs_neg_resp+=predicted_probs[indices_neg[i], 0].cpu().data.squeeze().numpy()

                if sum_neg == 0:
                    prob_loss = ((1 /sum_pos)*(Probs_neg_resp))*alpha

                elif sum_pos == 0:
                    prob_loss = ((1 /sum_neg)*(Probs_pos_resp))*alpha

                else:
                    prob_loss = (((1 / sum_pos) * (Probs_neg_resp)) + ((1 / sum_neg)*(Probs_pos_resp)))* alpha

                prob_loss = torch.from_numpy(np.array(prob_loss))
                out_confid_loss.append(prob_loss.detach().cpu().numpy())
                ...
                total_loss = kld_loss + bce_loss + classifier_loss_crt + prob_loss
                ...
                prob_loss_epoch += prob_loss.item()
            ...
            writer.add_scalar('Prob Loss/train', prob_loss_epoch / num_batches, epoch)
            ######################################################################
...
final.to_csv(
    r'{0}/_Five_Iterations.csv'.format(root_save_dir), header=True)