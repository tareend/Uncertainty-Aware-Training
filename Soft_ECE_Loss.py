...
def compute_calibration(pred_labels, true_labels, confidences, soft_binning_bins, soft_binning_decay_factor):

    soft_predictions_product = []
    correct_pred_labels = np.zeros(pred_labels.shape)
    true_labels = true_labels.cpu().data.squeeze().numpy()

    for i in range(pred_labels.shape[0]):
        if pred_labels[i] == true_labels[i]:
            correct_pred_labels[i] = 1
        else:
            correct_pred_labels[i] = 0

    correct_pred_labels = tf.convert_to_tensor(tf.dtypes.cast(correct_pred_labels, tf.float32))
    predictions = tf.convert_to_tensor(tf.dtypes.cast(confidences, tf.float32))
    ...
    anchors = tf.keras.backend.get_value(soft_binning_anchors)
    ...
    intermediate_predictions_reshaped_tensor = tf.tile(
        tf.expand_dims(predictions, 1), [1, tf.shape(soft_binning_anchors)[0]])

    intermediate_predictions_reshaped_tensor = tf.dtypes.cast(intermediate_predictions_reshaped_tensor, tf.float32)
    ...
    intermediate_labels_reshaped_tensor = tf.tile(
        tf.expand_dims(correct_pred_labels, 1), [1, tf.shape(soft_binning_anchors)[0]])

    intermediate_labels_reshaped_tensor = tf.dtypes.cast(intermediate_labels_reshaped_tensor, tf.float32)
    ...
    net_bin_accuracy = tf.divide(
        tf.reduce_sum(
            tf.multiply(intermediate_labels_reshaped_tensor,
                        predictions_soft_binning_coeffs),
            axis=[0]),
        tf.maximum(sum_coeffs_for_bin, tf.ones(sum_coeffs_for_bin.shape)))  # EPS * tf.ones(sum_coeffs_for_bin.shape)))
    ...
    return soft_binning_ece, predictions_soft_binning_coeffs, soft_binning_anchors
...
## Soft binning variables ##

soft_binning_use_decay = True
soft_binning_decay_factor = 0.5
soft_binning_temp = soft_binning_decay_factor # set in function from decay factor
soft_binning_bins = 15 # in paper set to 15 and T =0.01
bin_size = soft_binning_bins
EPS = 1e-5
...
for iteration in range(Iteration_Runs):
    ...
    save_dir = os.path.join(root_save_dir_iter,
                            'saved_weights_and_data_lr_{0}_nbF_{1}_codeDim_{2}_z_concat_dim_{3}_kld_{4}_classif_weight_{5}_beta_{6}'.format(
                                lr,
                                num_filters,
                                code_dim,
                                z_concat_dim,
                                kld_weight,
                                classifier_weight, beta))

    image_dir = os.path.join(root_save_dir_iter,
                             'images_lr_{0}_nbF_{1}_codeDim_{2}_z_concat_dim_{3}_kld_{4}_classif_weight_{5}_beta_{6}'.format(
                                 lr, num_filters,
                                 code_dim, z_concat_dim, kld_weight, classifier_weight, beta))
    ...
    for train_indices, test_indices in skf.split(indices, Y_CRT):
        ...
        for epoch in range(num_epochs):
            ...
            pred_labels_CRT_hard_label = np.zeros(N)
            ...
            for batch_idx in range(num_batches):
                ...
                pred_labels_CRT_hard_label[this_batch_indices] = np.argmax(predicted_probs.cpu().data.squeeze().numpy(), axis=1)
                ...
                soft_ece_loss_val, predictions_soft_binning_coeffs, soft_binning_anchors = compute_calibration(
                    pred_labels_CRT_hard_label[this_batch_indices], Y_CRT_train[this_batch_indices], confidences[this_batch_indices],
                    soft_binning_bins, soft_binning_decay_factor)

                ece_loss = tf.keras.backend.get_value(soft_ece_loss_val)
                tf.keras.backend.clear_session()
                soft_ece_loss = torch.tensor(ece_loss)

                ################################################################################################################################################

                total_loss = kld_loss + bce_loss + classifier_loss_crt + soft_ece_loss*beta
                ...
                sece_loss_epoch += soft_ece_loss.item()
            ...
            writer.add_scalar('SECE Loss/train', sece_loss_epoch / num_batches, epoch)
            ######################################################################
